...
use vex_config::{ConfigLoader, VexConfig, Environment};
use std::env;
...
"#;
    ...
    println!("Created override config: {}", override_config_path.display());
    ...
    println!("Authentication: {}", config.core_networking.enable_authentication);
    ...
    println!("Authentication: {}", base_config.core_networking.enable_authentication);
    ...
    unsafe { env::set_var("VEX_CORE_NETWORKING__INITIAL_PORT", "47001"); }
    unsafe { env::set_var("VEX_LOGGING__LEVEL", "trace"); }
    unsafe { env::set_var("VEX_GATEWAY_NETWORKING__MAX_MESSAGE_SIZE", "16384"); }
    ...
    println!("Port (from env): {}", config_with_env.core_networking.initial_port);
    println!("Logging level (from env): {}", config_with_env.logging.level);
    println!("Max message size (from env): {}", config_with_env.gateway_networking.max_message_size);

    // Clean up environment variables
    unsafe { env::remove_var("VEX_CORE_NETWORKING__INITIAL_PORT"); }
    unsafe { env::remove_var("VEX_LOGGING__LEVEL"); }
    unsafe { env::remove_var("VEX_GATEWAY_NETWORKING__MAX_MESSAGE_SIZE"); }
    ...
    // Customize logging
    custom_config.logging.set_module_level("networking".to_string(), 
        vex_config::logging::LogLevel::Trace);
    custom_config.logging.set_module_level("orderbook".to_string(), 
        vex_config::logging::LogLevel::Warn);
    custom_config.logging.add_custom_field("component".to_string(), "custom".to_string());
    
    println!("Custom configuration created:");
    println!("Core ID: {}", custom_config.core_networking.core_id);
    println!("Max gateways: {}", custom_config.core_networking.max_gateways);
    println!("Networking log level: {:?}", 
        custom_config.logging.get_module_level("networking"));
    println!("Orderbook log level: {:?}", 
