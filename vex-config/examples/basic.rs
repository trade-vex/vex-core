...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match VexConfig::load_auto() {
        Ok(config) => {
            ...
            println!("Core networking port: {}", config.core_networking.initial_port);
    ...
    for env in [Environment::Development, Environment::Test, Environment::Production] {
        println!("2. Loading configuration for {} environment...", env);
        
        let config = VexConfig::load_for_environment(env.clone())
            .unwrap_or_else(|_| {
        ...
        println!("Authentication enabled: {}", config.core_networking.enable_authentication);
    ...
    for env in [Environment::Development, Environment::Test, Environment::Production] {
        ...
        println!("     - Max connections per address: {}", config.core_networking.max_connections_per_address);
    }
