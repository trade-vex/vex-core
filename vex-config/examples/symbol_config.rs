...
use vex_config::{VexConfig, Environment, SymbolSpecificationConfig};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Loaded {} symbols for development environment", dev_config.symbols.len());
    ...
    println!("   Total symbols after merge: {}", full_config.symbols.len());
    println!("   Configuration validation: {}", 
        if full_config.validate().is_ok() { "✓ Valid" } else { "✗ Invalid" });
