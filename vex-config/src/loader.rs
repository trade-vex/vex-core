...
impl ConfigLoader {
    ...
    fn load_auto_for_environment(
        ...
        match self.load_for_environment(environment.clone()) {
            Ok(config) => Ok(config),
            // Only a genuinely ABSENT config file may fall back to defaults. A file that exists
            // but fails to parse, deserialize or validate is an operator error and must surface:
            // swallowing it silently substituted hardcoded defaults (different markets, scale
            // factors and fees) for the configuration the operator intended (finding C2-1).
            Err(ConfigError::NotFound(message))
                if environment.is_development() || environment.is_test() =>
            {
                tracing::warn!(
                    target: "config",
                    action = "config_file_absent",
