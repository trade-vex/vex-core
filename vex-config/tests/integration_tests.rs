...
use vex_config::{VexConfig, Environment, ConfigLoader, ConfigError};
...
fn test_environment_specific_defaults() {
    ...
    assert_eq!(dev_config.logging.level, vex_config::logging::LogLevel::Debug);
    assert_eq!(test_config.logging.level, vex_config::logging::LogLevel::Info);
    assert_eq!(prod_config.logging.level, vex_config::logging::LogLevel::Info);
}
...
fn test_file_loading_and_saving() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(loaded_config.core_networking.core_id, original_config.core_networking.core_id);
...
fn test_different_file_formats() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(toml_config.core_networking.core_id, config.core_networking.core_id);
    assert_eq!(json_config.core_networking.core_id, config.core_networking.core_id);
    assert_eq!(yaml_config.core_networking.core_id, config.core_networking.core_id);
...
fn test_environment_variable_overrides() -> Result<(), Box<dyn std::error::Error>> {
    ...
    unsafe { env::set_var("TEST_CORE_NETWORKING__INITIAL_PORT", "9999"); }
    unsafe { env::set_var("TEST_LOGGING__LEVEL", "error"); }
    unsafe { env::set_var("TEST_GATEWAY_NETWORKING__MAX_MESSAGE_SIZE", "32768"); }
    ...
    // Clean up
    unsafe { env::remove_var("TEST_CORE_NETWORKING__INITIAL_PORT"); }
    unsafe { env::remove_var("TEST_LOGGING__LEVEL"); }
    unsafe { env::remove_var("TEST_GATEWAY_NETWORKING__MAX_MESSAGE_SIZE"); }
...
fn test_configuration_merging() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_ne!(base_config.core_networking.enable_authentication, original_auth);
    ...
    assert_eq!(base_config.logging.level, vex_config::logging::LogLevel::Error);
...
fn test_logging_configuration() {
    ...
    config.logging.set_module_level("test_module".to_string(), 
        vex_config::logging::LogLevel::Trace);
    config.logging.set_module_level("another_module".to_string(), 
        vex_config::logging::LogLevel::Error);
    
    assert_eq!(config.logging.get_module_level("test_module"), 
        &vex_config::logging::LogLevel::Trace);
    assert_eq!(config.logging.get_module_level("another_module"), 
        &vex_config::logging::LogLevel::Error);
    assert_eq!(config.logging.get_module_level("unknown_module"), 
        &config.logging.level);
    
    // Test custom fields
    config.logging.add_custom_field("service".to_string(), "test".to_string());
    config.logging.add_custom_field("version".to_string(), "1.0.0".to_string());
    
    assert_eq!(config.logging.custom_fields.get("service"), Some(&"test".to_string()));
    assert_eq!(config.logging.custom_fields.get("version"), Some(&"1.0.0".to_string()));
    
    // Test log level checking
    assert!(config.logging.is_enabled(&vex_config::logging::LogLevel::Info, None));
    assert!(config.logging.is_enabled(&vex_config::logging::LogLevel::Trace, Some("test_module")));
    assert!(!config.logging.is_enabled(&vex_config::logging::LogLevel::Debug, Some("another_module")));
}

#[test]
fn test_environment_detection() {
    // Test with VEX_ENV
    unsafe { env::set_var("VEX_ENV", "production"); }
    assert_eq!(Environment::detect(), Environment::Production);
    unsafe { env::remove_var("VEX_ENV"); }
    
    // Test with ENVIRONMENT
    unsafe { env::set_var("ENVIRONMENT", "test"); }
    assert_eq!(Environment::detect(), Environment::Test);
    unsafe { env::remove_var("ENVIRONMENT"); }
    
    // Test with ENV
    unsafe { env::set_var("ENV", "dev"); }
...
fn test_auto_loading() {
    ...
    unsafe { env::set_var("VEX_ENV", "test"); }
