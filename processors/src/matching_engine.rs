...
impl MatchingEngineRouter {
    ...
    pub fn add_symbol(
        &mut self,
        symbol_id: u32,
        spec: common::model::symbol_specification::CoreSymbolSpecification,
        book_type: OrderBookImplType,
    ) {
...
mod tests {
    ...
    fn test_process_cancel_order() {
        ...
        // Cancel the order
        let mut cancel_cmd =
            create_test_order_command(100, 1, 0, 0, Side::Bid, OrderCommandType::CancelOrder, 1);
        ...
        for symbol_id in 0u64..16 {
            ...
            for (shard_idx, shard) in shards.iter().enumerate() {
                if shard_idx == expected_shard {
                    assert!(
                        shard.symbol_for_this_handler(symbol_id),
                        "Symbol {} should be handled by shard {}",
                        symbol_id,
                        shard_idx
                    );
                } else {
                    assert!(
                        !shard.symbol_for_this_handler(symbol_id),
                        "Symbol {} should NOT be handled by shard {}",
                        symbol_id,
                        shard_idx
                    );
                }
