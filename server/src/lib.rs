    ...
    if config.balance_preload.enabled {
        ...
        for (user_id, balances) in &config.balance_preload.users {
            for balance in balances {
                ...
                let sequence = loop {
                    if let Ok(sequence) = producer.try_publish(|cmd| {
                        *cmd =
                            OrderCommand::deposit_funds(*user_id, balance.amount, balance.asset_id);
                    }) {
                        break sequence;
                    }
                    std::hint::spin_loop();
                };
                engine.publications().submitted(sequence);
            }
        }
...
mod tests {
    ...
    fn test_quote_cost_at_shipped_eth_and_btc_scales() {
        ...
        assert_eq!(btc.calculate_quote_cost(btc_price, 1), Some(600));
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(18_446_744_073_709_551_600)
        );

        // One atomic unit beyond that no longer fits in u64: the cost must be REJECTED, not
        // saturated. A saturated u64::MAX is indistinguishable from a legitimate cost and lets
        // an order reserve far less collateral than it can owe (finding C4-1).
        assert_eq!(
            btc.calculate_quote_cost(btc_price, largest_btc_fill_in_range + 1),
            None
        );
        // And the intermediate product overflowing u128 must also be rejected, not wrapped:
        // the release profile has overflow-checks off, so an unchecked multiply wraps silently
        // into a small, plausible-looking cost.
        assert_eq!(eth.calculate_quote_cost(u64::MAX, u64::MAX), None);
    }
    ...
    fn test_shipped_btc_quote_cost_is_monotonic_past_largest_in_range() {
        ...
        assert!(
            largest_cost.is_some(),
            "the largest in-range fill must still price"
        );
        // Stronger than the original "must not wrap": one unit past the representable range is
        // now REJECTED rather than saturated. Saturating to u64::MAX was indistinguishable from
        // a legitimate cost and let an order under-reserve collateral (finding C4-1).
        assert_eq!(
            next_cost, None,
            "quote cost past the largest in-range fill must be rejected, not wrapped or saturated"
        );
    }
