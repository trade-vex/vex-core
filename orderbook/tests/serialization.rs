//! Tests for serialization and deserialization of the order book.
use common::model::enums::{OrderType, Side};
use common::model::symbol_specification::TestConstants;
use orderbook::naive_impl::OrderBookNaiveImpl;
use orderbook::{OrderBook, OrderCommand};

#[test]
fn test_naive_serialization_deserialization() {
    let mut order_book = OrderBookNaiveImpl::new(TestConstants::symbol_spec_eth_xbt());

    let cmd = OrderCommand::new_order(OrderType::Gtc, 1, 100, 1000, 0, 10, Side::Ask);
    order_book.new_order(&mut cmd.clone()).unwrap();

    let cmd2 = OrderCommand::new_order(OrderType::Gtc, 2, 101, 1001, 0, 5, Side::Ask);
    order_book.new_order(&mut cmd2.clone()).unwrap();

    let cmd3 = OrderCommand::new_order(OrderType::Gtc, 3, 102, 900, 0, 20, Side::Bid);
    order_book.new_order(&mut cmd3.clone()).unwrap();

    let bytes = order_book.to_bytes().unwrap();
    let mut reader = bytes.as_slice();
    let deserialized_book = OrderBookNaiveImpl::from_bytes(&mut reader).unwrap();

    assert_eq!(
        order_book.get_orders_num(Side::Ask),
        deserialized_book.get_orders_num(Side::Ask)
    );
    assert_eq!(
        order_book.get_orders_num(Side::Bid),
        deserialized_book.get_orders_num(Side::Bid)
    );
    assert_eq!(
        order_book.get_total_orders_volume(Side::Ask),
        deserialized_book.get_total_orders_volume(Side::Ask)
    );
    assert_eq!(
        order_book.get_total_orders_volume(Side::Bid),
        deserialized_book.get_total_orders_volume(Side::Bid)
    );
}
