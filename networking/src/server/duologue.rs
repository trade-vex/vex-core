...
use rusteron_client::{
    Aeron, AeronAvailableImageCallback, AeronCError, AeronFragmentHandlerCallback, AeronHeader,
    AeronImage, AeronNotificationLogger, AeronPublication, AeronReservedValueSupplierLogger,
...
impl AeronAvailableImageCallback for DuologueImageAvailable {
    fn handle_aeron_on_available_image(
        ...
        if remote_addr != self.owner {
            error!(
                "Client Connecting witht the wrong address, expected: {}, got: {}",
                self.owner, remote_addr
            );
        } else {
            info!(
