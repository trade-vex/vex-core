...
fn test_add_multiple_subscriptions() {
    ...
    assert!(subscriber.add_subscription("aeron:udp?endpoint=localhost:40123", 3003).is_ok());
...
fn test_receive_message() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 3005)
    ...
    if let Ok(messages) = received_messages.lock() {
        ...
        assert_eq!(messages[0], b"Hello, Subscriber!", "Received message does not match expected");
    } else {
...
fn test_receive_multiple_messages() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 3006)
        .expect("Failed to set up test subscriber");
    ...
    if let Ok(received) = received_messages.lock() {
        ...
        for (i, expected) in messages.iter().enumerate() {
            assert_eq!(&received[i], expected.as_bytes(), "Message {} does not match expected", i);
        }
    } else {
        panic!("Failed to access received messages");
    }
...
fn test_receive_from_multiple_subscriptions() {
    ...
    if let Ok(messages) = received_messages.lock() {
        ...
        assert!(messages.contains(&message1.to_vec()), "Message from stream 3007 not received");
        assert!(messages.contains(&message2.to_vec()), "Message from stream 3008 not received");
    } else {
        panic!("Failed to access received messages");
    }
...
fn test_receive_large_message() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 3009)
        .expect("Failed to set up test subscriber");
...
fn test_concurrent_publishers() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 3012)
        .expect("Failed to set up test subscriber");
    ...
    if let Ok(messages) = received_messages.lock() {
        assert_eq!(messages.len(), 2, "Expected 2 messages, got {}", messages.len());
        assert!(messages.contains(&message1.to_vec()), "Message from publisher 1 not received");
        assert!(messages.contains(&message2.to_vec()), "Message from publisher 2 not received");
    } else {
        panic!("Failed to access received messages");
    }
...
fn test_mixed_channel_types() {
    ...
    subscriber.add_subscription("aeron:udp?endpoint=localhost:40123", 3014).unwrap();
    subscriber.start();
    ...
    let mut udp_publisher = setup_test_publisher(&context_dir, "aeron:udp?endpoint=localhost:40123", 3014)
    ...
    udp_publisher.send(udp_message, "aeron:udp?endpoint=localhost:40123", 3014).unwrap();
    ...
    if let Ok(messages) = received_messages.lock() {
        assert_eq!(messages.len(), 2, "Expected 2 messages, got {}", messages.len());
        assert!(messages.contains(&ipc_message.to_vec()), "IPC message not received");
        assert!(messages.contains(&udp_message.to_vec()), "UDP message not received");
    } else {
        panic!("Failed to access received messages");
    }
...
fn test_message_ordering() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 3015)
        .expect("Failed to set up test subscriber");
    ...
    for message in &messages {
        publisher.send(message.as_bytes(), "aeron:ipc", 3015).unwrap();
    ...
    if let Ok(received) = received_messages.lock() {
        ...
        for (i, expected) in messages.iter().enumerate() {
            assert_eq!(&received[i], expected.as_bytes(), "Message {} does not match expected order", i);
        }
    } else {
        panic!("Failed to access received messages");
    }
