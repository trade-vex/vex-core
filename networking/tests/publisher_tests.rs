...
fn test_publisher_creation() {
    ...
    let context_dir = CString::new("/home/esciiee/vex/aeron/aeron-samples/scripts").expect("Failed to create CString");
    let (mut subscriber, _received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 2001)
...
fn test_add_publication() {
    ...
    let (mut subscriber, _received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 2002)
        .expect("Failed to set up test subscriber");
...
fn test_add_multiple_publications() {
    ...
    let (mut subscriber1, _received_messages1) = setup_test_subscriber(&context_dir, "aeron:ipc", 2003)
        .expect("Failed to set up test subscriber 1");
    let (mut subscriber2, _received_messages2) = setup_test_subscriber(&context_dir, "aeron:udp?endpoint=localhost:40123", 2004)
        .expect("Failed to set up test subscriber 2");
    let (mut subscriber3, _received_messages3) = setup_test_subscriber(&context_dir, "aeron:ipc", 2005)
        .expect("Failed to set up test subscriber 3");
    
    let mut publisher = AeronPublisher::new(&context_dir).unwrap();
    
    assert!(publisher.add_publication("aeron:ipc", 2003).is_ok());
    assert!(publisher.add_publication("aeron:udp?endpoint=localhost:40123", 2004).is_ok());
...
fn test_send_message() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 2006)
        .expect("Failed to set up test subscriber");
    ...
    if let Ok(messages) = received_messages.lock() {
        ...
        assert_eq!(messages[0], b"Hello, Aeron!", "Received message does not match expected");
    } else {
...
fn test_send_message_to_nonexistent_publication() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, _received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 2007)
        .expect("Failed to set up test subscriber");
    ...
    if let Ok(messages) = _received_messages.lock() {
        assert!(messages.is_empty(), "Messages were still received when no publication exists");
    } else {
        panic!("Failed to access received messages");
    }
...
fn test_send_empty_message() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 2008)
        .expect("Failed to set up test subscriber");
...
fn test_send_all_messages() {
    ...
    let (mut subscriber1, received_messages1) = setup_test_subscriber(&context_dir, "aeron:ipc", 2009)
        .expect("Failed to set up test subscriber 1");
    let (mut subscriber2, received_messages2) = setup_test_subscriber(&context_dir, "aeron:ipc", 2010)
        .expect("Failed to set up test subscriber 2");
    ...
    if let Ok(messages) = received_messages1.lock() {
        assert!(!messages.is_empty(), "No messages were received");
        assert_eq!(messages[0], b"Broadcast message", "Received message does not match expected");
    } else {
        panic!("Failed to access received messages");
    }
...
fn test_send_all_empty_message() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 2011)
        .expect("Failed to set up test subscriber");
...
fn test_stop_publisher() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, _received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 2012)
        .expect("Failed to set up test subscriber");
...
fn test_send_after_stop() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, _received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 2013)
        .expect("Failed to set up test subscriber");
...
fn test_large_message_send() {
    ...
    let context_dir = CString::new("/tmp/aeron-test").expect("Failed to create CString");
    let (mut subscriber, received_messages) = setup_test_subscriber(&context_dir, "aeron:ipc", 2014)
        .expect("Failed to set up test subscriber");
    ...
    if let Ok(messages) = received_messages.lock() {
        assert!(!messages.is_empty(), "No messages were received");
        assert_eq!(messages[0], large_message, "Received message does not match expected");
    }
