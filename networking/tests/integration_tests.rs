...
fn test_multiple_udp_endpoints() {
    ...
    if let Ok(messages) = received_messages.lock() {
        ...
        assert!(
            ...
            "Message from endpoint 1 not received"
        );
        assert!(
            messages.contains(&message2.to_vec()),
            "Message from endpoint 2 not received"
        );
    } else {
        panic!("Failed to access received messages");
    }
...
fn test_udp_broadcast_simulation() {
    ...
    let (mut subscriber1, received_messages1) =
        setup_test_subscriber(&context_dir, &channel, 4004).unwrap();
    let (mut subscriber2, received_messages2) =
        setup_test_subscriber(&context_dir, &channel, 4004).unwrap();
    let (mut subscriber3, received_messages3) =
        setup_test_subscriber(&context_dir, &channel, 4004).unwrap();
    ...
    .enumerate()
    {
        if let Ok(messages) = received_messages.lock() {
            ...
            assert_eq!(
                messages[0],
                message,
                "Subscriber {} received wrong message",
                i + 1
            );
        } else {
            panic!(
                "Failed to access received messages for subscriber {}",
                i + 1
            );
        }
    }
...
fn test_udp_large_message_transfer() {
    ...
    let (mut subscriber, received_messages) =
        setup_test_subscriber(&context_dir, &channel, 4005).unwrap();
...
fn test_udp_high_frequency_messages() {
    ...
    let (mut subscriber, received_messages) =
        setup_test_subscriber(&context_dir, &channel, 4006).unwrap();
    ...
    if let Ok(received) = received_messages.lock() {
        assert!(
            received.len() >= message_count * 9 / 10,
            "Expected at least 90% of messages, got {}",
            received.len()
        );

        // Verify message ordering (most should be in order)
        let mut in_order_count = 0;
        for (i, received_msg) in received
            .iter()
            .take(message_count.min(received.len()))
            .enumerate()
        {
        ...
        assert!(
            in_order_count >= message_count * 8 / 10,
            "Too many messages out of order"
        );
    } else {
        panic!("Failed to access received messages");
    }
...
fn test_udp_network_partition_simulation() {
    ...
    let (mut subscriber, received_messages) =
        setup_test_subscriber(&context_dir, &channel, 4007).unwrap();
    ...
    // Restart subscriber
    let (mut new_subscriber, new_received_messages) =
        setup_test_subscriber(&context_dir, &channel, 4007).unwrap();
    ...
    if let Ok(messages) = received_messages.lock() {
        assert_eq!(
            messages.len(),
            1,
            "Original subscriber should have only 1 message"
        );
        assert_eq!(
            messages[0], message1,
            "Original subscriber message mismatch"
        );
    }
    ...
    if let Ok(messages) = new_received_messages.lock() {
        assert_eq!(
            messages.len(),
            2,
            "New subscriber should have only 2 message"
        );
...
fn test_udp_message_fragmentation() {
    ...
    let (mut subscriber, received_messages) =
        setup_test_subscriber(&context_dir, &channel, 4009).unwrap();
    ...
    if let Ok(messages) = received_messages.lock() {
        ...
        assert_eq!(
            messages[0].len(),
            large_message_size,
            "Fragmented message size mismatch"
        );
        assert_eq!(
            messages[0], large_message,
            "Fragmented message content mismatch"
        );
    } else {
        panic!("Failed to access received messages");
    }
...
fn test_udp_concurrent_publishers_subscribers() {
    ...
    for (i, message) in test_messages.iter().enumerate() {
        ...
        publishers[publisher_idx]
            .send(message, &channel, 4010)
            .unwrap();
    ...
    for (subscriber_idx, received_messages) in received_messages.iter().enumerate() {
        if let Ok(messages) = received_messages.lock() {
            assert!(
                !messages.is_empty(),
                "Subscriber {} received no messages",
                subscriber_idx
            );
            assert!(
                messages.len() >= 2,
                "Subscriber {} received too few messages",
                subscriber_idx
            );
        }
    }
...
fn test_udp_performance_characteristics() {
    ...
    let (mut subscriber, received_messages) =
        setup_test_subscriber(&context_dir, &channel, 4013).unwrap();
    ...
    if let Ok(messages) = received_messages.lock() {
        ...
        println!(
            "UDP Performance: {} messages in {:?} ({:.2} msg/sec)",
            received_count, duration, throughput
        );

        assert!(
            received_count >= message_count * 8 / 10,
            "Expected at least 80% message delivery, got {}",
            received_count
        );
    }
...
fn test_udp_mixed_message_sizes() {
    ...
    let (mut subscriber, received_messages) =
        setup_test_subscriber(&context_dir, &channel, 4014).unwrap();
